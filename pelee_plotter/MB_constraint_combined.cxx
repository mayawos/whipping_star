...
int main(int argc, char* argv[])
{
	...
	CollapseSubchannels(Mtotalfraccov, Mcolfrac, num_bins, num_channels, num_subchannels);
        ...
        plot_one(Mcol0, h_nue_total_nolee, h_numu, "SBNfit_covariance_matrix_nolee"+tag1+"_mc");
        plot_one(Mcolfrac, h_nue_total_nolee, h_numu, "SBNfit_fractional_covariance_matrix_"+tag1+"_mc");
        plot_one(Mcolcorr, h_nue_total_nolee, h_numu, "SBNfit_correlation_matrix_"+tag1+"_mc");
	...
	// Mcol(i,i) += input_vec_mc_total[i];
	  ...
	  if( bin > (h_nue_total_nolee->GetNbinsX()-1) ){ 
	    ...
	    std::cout << "###numu, err  = " << (*Mcol2)[bin][bin] << ", " << h_numu->GetBinContent(bin+1) << ", " << derr/h_numu->GetBinContent(bin+1) << std::endl;
	  }else{
	    double derr = (*Mcol2)[bin][bin] > 0 ? sqrt((*Mcol2)[bin][bin]): 0.; 
	...
	if (!svd.Decompose() ) {
	  std::cout << "With lee" << std::endl;
	  std::cout << "Decomposition failed, matrix singular ?" << std::endl;
	}else{
	  std::cout << "I am inverting the matrix" << std::endl;
	  for( int i = 0; i < InvertedMatrix.GetNcols(); i++ ){
	    for( int j = 0; j < InvertedMatrix.GetNrows(); j++ ){
	      if(i==j) std::cout << "before InvertedMatrix " << i << ", " << j << " = " << (InvertedMatrix)(i,j) << std::endl;
	    }
	  }
	  InvertedMatrix = Mcol.Invert();
	  for( int i = 0; i < InvertedMatrix.GetNcols(); i++ ){
	    for( int j = 0; j < InvertedMatrix.GetNrows(); j++ ){
	      if(i==j) std::cout << "after InvertedMatrix " << i << ", " << j << " = " << (InvertedMatrix)(i,j) << std::endl;
	    }
	  }
