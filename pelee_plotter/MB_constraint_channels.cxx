...
int main(int argc, char* argv[])
{
	...
	TObject *obj;
	  ...
	  input_vec_mc_total_nolee.push_back(h_numu->GetBinContent(b));
        ...
        //fname = "../bin/SBNfit_variation_plots_"+tag2+".root";
        TFile *f_data = new TFile(fname,"read");
        TH1D *h_nue_fake_data;
        if(tag2.find("nue_numu") != std::string::npos ){ 
            h_nue_fake_data = (TH1D*)f_data->Get("nu_uBooNE_nue_data");
            //h_nue_fake_data = (TH1D*)f_data->Get("All_Genie/nu_uBooNE_nue_intrinsic_72");
        }else{
            h_nue_fake_data = (TH1D*)f_data->Get("nu_uBooNE_1e0p_data");
            //h_nue_fake_data = (TH1D*)f_data->Get("All_Genie/nu_uBooNE_1e0p_intrinsic_72");
        }
	...
	CollapseSubchannels(Mtotalcorr, Mcolcorr, num_bins, num_channels, num_subchannels);
        ...
        TMatrixD numumatrix(h_nue_total_nolee->GetNbinsX(), h_nue_total_nolee->GetNbinsX());
        for( int i = 0; i < Mcol.GetNcols(); i++ ){
        for( int j = 0; j < Mcol.GetNcols(); j++ ){
          if( i < h_nue_total_nolee->GetNbinsX() && j < h_nue_total_nolee->GetNbinsX() ){ 
            nuematrix(i,j)  = Mcol(i,j);
            nuematrix_nolee(i,j)  = Mcol0(i,j);
          }
          else numumatrix = Mcol(i,j);
          }
        }

        for( int i = 0; i < Mcol.GetNcols(); i++ ){
        for( int j = 0; j < Mcol.GetNcols(); j++ ){
           if( i==j && i >= h_nue_total_nolee->GetNbinsX() && j >= h_nue_total_nolee->GetNbinsX() ) Mcol(i,j) += h_numu_data->GetBinContent((i - h_nue_total_nolee->GetNbinsX()) + 1);
           if( i==j && i >= h_nue_total_nolee->GetNbinsX() && j >= h_nue_total_nolee->GetNbinsX() ) Mcol0(i,j) += h_numu_data->GetBinContent((i - h_nue_total_nolee->GetNbinsX()) + 1);
        }
        }
        ...
        for( int bin = 0; bin < Mcol.GetNcols(); bin++ ){
           if( bin > (h_nue_total_nolee->GetNbinsX()-1) ){ 
             ...
             double derr = Mcol(bin,bin) > 0 ? sqrt(Mcol(bin,bin)): 0.; 
             double derr_nolee = Mcol0(bin,bin) > 0 ? sqrt(Mcol0(bin,bin)): 0.; 
             ...
             std::cout << "before const nue nolee, err  = " << derr_nolee/h_nue_total_nolee->GetBinContent(bin+1) << std::endl;
           }	
        }

        TString cname = tag1+"_nue_before_data_constraint";
        ...
        cname = tag1+"_numu_before_data_constraint";
	...
	for( int i = 0; i < InvertedMatrix.GetNcols(); i++ ){
	  for( int j = 0; j < InvertedMatrix.GetNrows(); j++ ){
	    if( (i==j) && i > (h_nue_total_nolee->GetNbinsX()-1) ) (InvertedMatrix)(i,j) += (1/input_vec_data[i]);
            if(i==j) std::cout << "Mcol " << i << ", " << j << " = " << (Mcol)(i,j) << std::endl;
            if(i==j) std::cout << "InvertedMatrix " << i << ", " << j << " = " << (InvertedMatrix)(i,j) << std::endl;
	  }
	}
	//InvertedMatrix.Print();
	
	//Invert back the matrix
	TMatrixD ConstrainedMatrix = InvertedMatrix;
	//Mcol2.SetTol(1e-50); 
	ConstrainedMatrix.Zero();
	TDecompSVD svd2( InvertedMatrix );
	if (!svd2.Decompose()) {
	  std::cout << "Decomposition failed, matrix singular ?" << std::endl;
	}else{
	  ConstrainedMatrix = InvertedMatrix.Invert();
	}

	//ConstrainedMatrix.Print();
        TMatrixD nuematrixconst(h_nue_total_nolee->GetNbinsX(), h_nue_total_nolee->GetNbinsX());
        TMatrixD numumatrixconst(h_nue_total_nolee->GetNbinsX(), h_nue_total_nolee->GetNbinsX());
        for( int i = 0; i < Mcol.GetNcols(); i++ ){
        for( int j = 0; j < Mcol.GetNcols(); j++ ){
           if( i < h_nue_total_nolee->GetNbinsX() && j < h_nue_total_nolee->GetNbinsX() ) nuematrixconst  = ConstrainedMatrix(i,j);
           else numumatrixconst = ConstrainedMatrix(i,j);
        }
        }
	
	//Invert covariance matrix        
	TMatrixD MCInvertedMatrix = *Mcol2;
	MCInvertedMatrix.Zero();
	TDecompSVD svdmc( *Mcol2 );
	if (!svdmc.Decompose()) {
	  std::cout << "Decomposition failed, matrix singular ?" << std::endl;
	}else{
	  MCInvertedMatrix = Mcol2->Invert();
	}
	//MCInvertedMatrix.Print();
	//Adding the mc pull term contributions
	for( int i = 0; i < MCInvertedMatrix.GetNcols(); i++ ){
	  for( int j = 0; j < MCInvertedMatrix.GetNrows(); j++ ){
	    if( (i==j) && i > (h_nue_total->GetNbinsX()-1) ) (MCInvertedMatrix)(i,j) += (1/input_vec_mc_total[i]);
            if(i==j && i==0) std::cout << "ConstrainedMatrix " << i << ", " << j << " = " << (ConstrainedMatrix)(i,j) << std::endl;
            //if(i==j) std::cout << "MCInvertedMatrix " << i << ", " << j << " = " << (MCInvertedMatrix)(i,j) << std::endl;
	  }
	}
	
	//Finally, multiply all the matrix to solve the equations!
	TVectorD Nfit(ConstrainedMatrix.GetNcols());
	TMatrixD ScalingMatrix(ConstrainedMatrix.GetNcols(),ConstrainedMatrix.GetNrows());
	ScalingMatrix.Mult(ConstrainedMatrix,MCInvertedMatrix);
	//ScalingMatrix.Print();
	TVectorD CV(ConstrainedMatrix.GetNcols());
	for (int i=0; i<ConstrainedMatrix.GetNcols(); i++){
	  CV(i)=input_vec_mc_total[i];
	} 
	Nfit = ScalingMatrix*CV; 

	//invert the covariance matrix
	//Mcol.SetTol(1e-50); 
	TMatrixD InvertedMatrix0 = Mcol;
	InvertedMatrix0.Zero();
	TMatrixD InvMatrix0 = Mcol0;
	InvMatrix0.Zero();
	TDecompSVD svd0( Mcol0 );

	if (!svd0.Decompose() ) {
	  std::cout << "Decomposition failed, matrix singular ?" << std::endl;
	}else{
	  InvertedMatrix0 = Mcol0.Invert();
	  InvMatrix0 = InvertedMatrix0;
	}
	
	//InvertedMatrix.Print();
	///Adding the pull term contributions
	for( int i = 0; i < InvertedMatrix0.GetNcols(); i++ ){
	  for( int j = 0; j < InvertedMatrix0.GetNrows(); j++ ){
	    if( (i==j) && i > (h_nue_total_nolee->GetNbinsX()-1) ) (InvertedMatrix0)(i,j) += (1/input_vec_data[i]);
            if(i==j) std::cout << "Mcol0 " << i << ", " << j << " = " << (Mcol0)(i,j) << std::endl;
            if(i==j) std::cout << "InvertedMatrix0 " << i << ", " << j << " = " << (InvertedMatrix0)(i,j) << std::endl;
	  }
	}
	//Invert back the matrix
	TMatrixD ConstrainedMatrix_nolee = InvertedMatrix0;
	//Mcol2.SetTol(1e-50); 
	ConstrainedMatrix_nolee.Zero();
	TDecompSVD svd3( InvertedMatrix0 );
	if (!svd3.Decompose()) {
	  std::cout << "Decomposition failed, matrix singular ?" << std::endl;
	}else{
	  ConstrainedMatrix_nolee = InvertedMatrix0.Invert();
	}

	//Invert covariance matrix        
	TMatrixD MCInvertedMatrix_nolee = *Mcol3;
	MCInvertedMatrix_nolee.Zero();
	TDecompSVD svdmcnolee( *Mcol3 );
	if (!svdmcnolee.Decompose()) {
	  std::cout << "Decomposition failed, matrix nolee singular ?" << std::endl;
	}else{
	  MCInvertedMatrix_nolee = Mcol3->Invert();
	}
	//MCInvertedMatrix.Print();
	//Adding the mc pull term contributions
	for( int i = 0; i < MCInvertedMatrix_nolee.GetNcols(); i++ ){
	  for( int j = 0; j < MCInvertedMatrix_nolee.GetNrows(); j++ ){
	    if( (i==j) && i > (h_nue_total_nolee->GetNbinsX()-1) ) (MCInvertedMatrix_nolee)(i,j) += (1/input_vec_mc_total_nolee[i]);
            if(i==j && i==0) std::cout << "ConstrainedMatrix_nolee " << i << ", " << j << " = " << (ConstrainedMatrix_nolee)(i,j) << std::endl;
            //if(i==j) std::cout << "MCInvertedMatrix_nolee " << i << ", " << j << " = " << (MCInvertedMatrix_nolee)(i,j) << std::endl;
	  }
	}
	
	//Finally, multiply all the matrix to solve the equations!
	TVectorD Nfit_nolee(ConstrainedMatrix.GetNcols());
	TMatrixD ScalingMatrix_nolee(ConstrainedMatrix.GetNcols(),ConstrainedMatrix.GetNrows());
	ScalingMatrix_nolee.Mult(ConstrainedMatrix,MCInvertedMatrix);
	ScalingMatrix.Print();
	TVectorD CV_nolee(ConstrainedMatrix.GetNcols());
	for (int i=0; i<ConstrainedMatrix.GetNcols(); i++){
	  CV_nolee(i)=input_vec_mc_total_nolee[i];
	} 
	Nfit_nolee = ScalingMatrix_nolee*CV_nolee; 

        //clone the nue and numu scaled
        TH1D *h_numu_scaled = (TH1D*)h_numu->Clone("h_numu_scaled");
        h_numu_scaled->Reset(); 
        TH1D *h_nue_scaled = (TH1D*)h_nue_total->Clone("h_nue_scaled");
        TH1D *h_nue_scaled_nolee = (TH1D*)h_nue_total_nolee->Clone("h_nue_scaled_nolee");
        h_nue_scaled->Reset(); 
        h_nue_scaled_nolee->Reset(); 

        for( int bin = 0; bin < Nfit.GetNrows(); bin++ ){
           if( bin > (h_nue_total->GetNbinsX()-1) ){
             int numubin = bin-h_nue_total->GetNbinsX()+1; 
             h_numu_scaled->SetBinContent(numubin, Nfit[bin]);
             double derr = ConstrainedMatrix(bin,bin) > 0 ? sqrt(ConstrainedMatrix(bin,bin)): 0.; 
             double onlysyserr = sqrt(fabs(derr*derr - h_numu_scaled->GetBinContent(numubin))/(h_numu_scaled->GetBinContent(numubin)*h_numu_scaled->GetBinContent(numubin)));
             std::cout << "derr, h_numu_scaled->GetBinContent(numubin)), onlysyserr = " << derr << ", " << h_numu_scaled->GetBinContent(numubin) << ", " << onlysyserr << std::endl;
             h_numu_scaled->SetBinError(numubin,derr);
           }else{
             h_nue_scaled->SetBinContent(bin+1, Nfit[bin]);
             h_nue_scaled_nolee->SetBinContent(bin+1, Nfit_nolee[bin]);
             double derr = ConstrainedMatrix(bin,bin) > 0 ? sqrt(ConstrainedMatrix(bin,bin)): 0.;
             double derr_nolee = ConstrainedMatrix_nolee(bin,bin) > 0 ? sqrt(ConstrainedMatrix_nolee(bin,bin)): 0.;
             double onlysyserr = sqrt(fabs(derr*derr - h_nue_scaled->GetBinContent(bin+1))/(h_nue_scaled->GetBinContent(bin+1)*h_nue_scaled->GetBinContent(bin+1)));
             h_nue_scaled->SetBinError(bin+1,derr);
             double onlysyserr_nolee = sqrt(fabs(derr*derr - h_nue_scaled_nolee->GetBinContent(bin+1))/(h_nue_scaled_nolee->GetBinContent(bin+1)*h_nue_scaled_nolee->GetBinContent(bin+1)));
             h_nue_scaled_nolee->SetBinError(bin+1,derr_nolee);
             //std::cout << "derr^2, h_nue_scaled_nolee->GetBinContent(bin+1)), onlysyserr = " << derr*derr << ", " << h_nue_scaled_nolee->GetBinContent(bin+1) << ", " << onlysyserr_nolee << std::endl;
             std::cout << "nue scaled bin, err  = " << bin+1 << ", " << derr/h_nue_scaled->GetBinContent(bin+1) << std::endl;
             std::cout << "nue scaled bin, err  nolee = " << bin+1 << ", " << derr_nolee/h_nue_scaled_nolee->GetBinContent(bin+1) << std::endl;
             std::cout << "bin, h_nue_scaled, h_nue_scaled_err = " << bin+1 << ", " << h_nue_scaled->GetBinContent(bin+1) << ", " << h_nue_scaled->GetBinError(bin+1) << std::endl;
             std::cout << "bin, h_nue_scaled_nolee, h_nue_scaled_err_nolee = " << bin+1 << ", " << h_nue_scaled_nolee->GetBinContent(bin+1) << ", " << h_nue_scaled_nolee->GetBinError(bin+1) << std::endl;
           }	
        }
        //for(int h = 0; h < nues.size(); h++){
        //   if( std::string(nues[h]->GetName()).find("lee") != std::string::npos )h_nue_scaled->Add(nues[h]);
        //}
       
        std::vector<double> input_vec_scaled, input_vec_scaled_nolee;
        for(int b = 1; b < h_nue_scaled->GetNbinsX()+1; b++) input_vec_scaled.push_back(h_nue_scaled->GetBinContent(b));
        for(int b = 1; b < h_nue_scaled_nolee->GetNbinsX()+1; b++) input_vec_scaled_nolee.push_back(h_nue_scaled_nolee->GetBinContent(b));
        for(int b = 1; b < (h_numu_scaled->GetNbinsX()+1); b++){ 
          input_vec_scaled.push_back(h_numu_scaled->GetBinContent(b));
          input_vec_scaled_nolee.push_back(h_numu_scaled->GetBinContent(b));
        }

        cname = tag1+"_scaled_numu";
        if(!sys)cname = tag1+"_scaled_numu_noleesyst";
        DrawDataMCAndSyst(cname, h_numu_scaled, h_numu_scaled, h_numu_data, var, "#nu_{#mu} Selection");	
        cname = tag1+"_scaled_nue";
        if(!sys)cname = tag1+"_scaled_nue_noleesyst";
        DrawMCAndSyst(cname, h_nue_scaled, var, "#nu_{e} Selection");	
        cname = tag1+"_univ_overlay_nue";
        if(!sys)cname = tag1+"_univ_overlay_nue_noleesyst";
        std::cout << "Draw Data and MC Syst nue" << std::endl;
        DrawDataMCAndSyst(cname, h_nue_scaled, h_nue_scaled_nolee, h_nue_fake_data, var, "#nu_{e} Selection");	
        cname = tag1+"_overlay_nue";
        if(!sys)cname = tag1+"_overlay_nue_noleesyst";
        DrawMCAndSystOverlay(cname, h_nue_total, h_nue_scaled, var, "#nu_{e} Selection");	
        cname = tag1+"_overlay_nue_nolee";
        if(!sys)cname = tag1+"_overlay_nue_nolee_noleesyst";
        DrawMCAndSystOverlay(cname, h_nue_total_nolee, h_nue_scaled_nolee, var, "#nu_{e} Selection");
	
        //create root file containing the constrained matrix
	TFile * fconstr = new TFile(Form("constrained_%s.SBNcovar.root",tag1.c_str()),"recreate");	
        ...
        for( int i = 0; i < ConstMatrix.GetNcols(); i++ ){
          for( int j = 0; j < ConstMatrix.GetNrows(); j++ ){
            ConstMatrix(i,j) = ConstrainedMatrix(i,j);
            FracConstMatrix(i,j) = ConstrainedMatrix(i,j)/(input_vec_scaled[i]*input_vec_scaled[j]);
            //std::cout << "FracConstMatrix " << i << ", " << j << " = " << sqrt(FracConstMatrix(i,j)) << std::endl;
            CorrConstMatrix(i,j) = ConstrainedMatrix(i,j)/(sqrt(ConstrainedMatrix(i,i))*sqrt(ConstrainedMatrix(j,j)));
          }
        }
        ...
        plot_one(ConstMatrix, h_nue_total, h_numu, "SBNfit_constrained_covariance_matrix_"+tag1+"_mc");
        plot_one(FracConstMatrix, h_nue_total, h_numu, "SBNfit_constrained_fractional_covariance_matrix_"+tag1+"_mc");
        plot_one(CorrConstMatrix, h_nue_total, h_numu, "SBNfit_constrained_correlation_matrix_"+tag1+"_mc");

        fconstr->Close();	
        //create root file containing the lee
        std::cout << "write TH1D h_nue_scaled" << std::endl;

	TFile * fsignal = new TFile(Form("constrained_%s_signal.SBNspec.root",tag1.c_str()),"recreate");	
        (TH1D*)h_nue_scaled->Write("nu_uBooNE_nue_sig"); 
        fsignal->Close();
	
	TFile * fbkg = new TFile(Form("constrained_%s_bkg.SBNspec.root",tag1.c_str()),"recreate");
        (TH1D*)h_nue_scaled_nolee->Write("nu_uBooNE_nue_sig"); 
        fbkg->Close();
