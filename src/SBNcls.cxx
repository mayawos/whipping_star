...
int SBNcls::CalcCLS(int numMC, std::string tag){
    ...
    std::vector<CLSresult> h0h0_results;

    std::cout<<"SBNcls::CalcCLS\t|| Starting H1 PDF generation "<<std::endl;
    std::vector<float> chi_datah1;
    if(which_mode==0){
    ...
    for(int i=0; i< h1_results.size(); i++){
        ...
        for(int p=0; p< prob_values.size();p++){
            std::cout<<"H1 Quantile @ "<<prob_values[p]<<" is "<<h1_results[i].m_quantiles[p]<<" for "<<h1_results[i].m_tag<<std::endl;
        }
    }
    ...
    for(int i=0; i< h0_results.size(); i++){
        h0_results[i].m_quantiles.resize(prob_values.size(),0.0);	
        h0_results[i].m_pdf.ComputeIntegral(); 
        h0_results[i].m_pdf.GetQuantiles(prob_values.size(), &(h0_results[i].m_quantiles)[0], &prob_values[0]);
        for(int p=0; p< prob_values.size();p++){
            std::cout<<"H0 Quantile @ "<<prob_values[p]<<" is "<<h0_results[i].m_quantiles[p]<<" for "<<h0_results[i].m_tag<<std::endl;
        }
    }
    ...
    for(int l=0; l < 3; l++){
      ...
      outfile << "==================================" << std::endl;
      for(int p=0; p < 5; p++) outfile << "delta chi2 H1, p-values    : " << h1_results[l].m_quantiles[p] << ", " << h0_results[l].m_nlower[p] << ", " << pval2sig(h0_results[l].m_nlower[p]) << std::endl;
      outfile << "==================================" << std::endl;
      outfile << nice_names[l] << " H0 median, std dev " << std::endl;
      outfile << "==================================" << std::endl;
      for(int p=0; p < 5; p++) outfile << "delta chi2 H0, p-values    : " << h0_results[l].m_quantiles[p] << ", " << h0h0_results[l].m_nlower[p] << ", " << pval2sig(h0h0_results[l].m_nlower[p]) <<std::endl;
      outfile << "==================================" << std::endl;
    }
    outfile << " CNP H1 median, std dev " << std::endl;
    outfile << "==================================" << std::endl;
    for(int p=0; p < 5; p++) outfile << "chi2 H1, p-values    : " << h1_results[3].m_quantiles[p] << ", " << h0_results[3].m_nlower[p] << ", " << pval2sig(h0_results[3].m_nlower[p]) << std::endl;
    for(int p=0; p < 5; p++) outfile << "chi2 H0, p-values    : " << h0_results[3].m_quantiles[p] << ", " << h0h0_results[3].m_nlower[p] << ", " << pval2sig(h0h0_results[3].m_nlower[p]) << std::endl;
    outfile << "==================================" << std::endl;
    outfile << " CNP H0 median, std dev " << std::endl;
    outfile << "==================================" << std::endl;
    for(int p=0; p < 5; p++) outfile << "chi2 H1, p-values    : " << h1_results[4].m_quantiles[p] << ", " << h0_results[4].m_nlower[p] << ", " << pval2sig(h0_results[4].m_nlower[p]) << std::endl;
    for(int p=0; p < 5; p++) outfile << "chi2 H0, p-values    : " << h0_results[4].m_quantiles[p] << ", " << h0h0_results[4].m_nlower[p] << ", " << pval2sig(h0h0_results[4].m_nlower[p]) << std::endl;
    outfile << "==================================" << std::endl;
    outfile.close();
    return 0 ;
}
...
int SBNcls::makePlotsFakedata(CLSresult &h0_result, CLSresult & h1_result, float chi2data, float pval_data, std::string tag, int which_mode){
    ...
    if(which_mode==1){
      //lets do CLs
      for(int p=0; p<pval.size();p++){
        std::cout << "pval.at(p), prob_values.at(p) = " << pval.at(p) << ", " << prob_values.at(p) << std::endl;
        vec_CLs.push_back(pval.at(p)/(1-prob_values.at(p)) );
      }
    }
    TFile * fp = new TFile(("SBNfit_CLs_"+tag+".root").c_str(),"recreate");
    ...
    h0_pdf.GetXaxis()->SetRangeUser(-100.,100.);
    h0_pdf.Draw("hist");

    h0_pdf.SetTitle("MicroBooNE Simulation, Preliminary");
    //h0_pdf.SetTitle(tag.c_str());
	...
	double sigma_val = pval2sig(pval.at(i));
	std::string whatsigma = to_string_prec(sigma_val,1)+"#sigma";
	double sigmadata_val = pval2sig(pval_data);
	std::string whatsigma_data = to_string_prec(sigmadata_val,1)+"#sigma";
	if(sigma_val==0.0){
	  whatsigma = "inf ";
	}
	if(sigmadata_val==0.0){
	  whatsigma_data = "inf ";
	}
	...
	std::string a_string_data;
	if(pval_data>0.001){
	  a_string_data = to_string_prec(pval_data,3);
	}else{
	  a_string_data = "10^{"+to_string_prec(log10(pval_data),2)+"}";
	}
...
//    std::string cv_details =  ("#splitline{CV}{#alpha("+ to_string_prec(pval.back(),5) +" | "+to_string_prec(pval2sig(pval.back()),5)+ "#sigma)}");
 //   std::cout<<cv_details<<std::endl;	
    ...
    if(draw_both&&which_mode==1) h1_pdf.Draw("hist same");	
...
/*int SBNcls::makeSimplePlots(TH1D & h0_pdf, TH1D & h1_pdf, std::string tag, std::vector<double> & pval, int which_mode){
        std::vector<double> ven={0};
	std::vector<double>  vec_CLs;
