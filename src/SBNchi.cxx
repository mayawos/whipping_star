 ...
 * ********************************************/
  ...
  //Standard chi^2 calculation constant
  double SBNchi::CalcChi(const SBNspec &sigSpec) const {
    double tchi = 0;
	...
	tchi += (core_spectrum.collapsed_vector.at(i)-sigSpec.collapsed_vector.at(i))*vec_matrix_inverted.at(i).at(j)*(core_spectrum.collapsed_vector.at(j)-sigSpec.collapsed_vector.at(j));
      }
    }
  ...
  TH1D SBNchi::SampleCovarianceVaryInput(SBNspec *specin, int num_MC, std::vector<double> * chival){
    ...
    int local_num_bins_total_compressed = num_bins_total_compressed;
    int local_num_MC = num_MC;
    int local_num_chival = num_chival;
...
#pragma acc parallel loop private(state,				\
  ...
  a_vec_matrix_lower_triangular[:local_num_bins_total][:local_num_bins_total],\ 
    a_vec_matrix_inverted[:local_num_bins_total_compressed][:local_num_bins_total_compressed], \
      ...
      this->a_num_subchannels[:local_num_channels])			\    
...
#pragma acc parallel loop						\
  ...
  copyout(a_vec_chis[:local_num_MC])		\
	...
	sampled_fullvector[a] = k - 1;
      }
#else
      for(int a=0; a<local_num_bins_total; a++) {
	sampled_fullvector[a] = rangen->Poisson(a_specin[a]); 
      }      
#endif
